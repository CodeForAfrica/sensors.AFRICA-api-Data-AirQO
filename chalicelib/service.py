import json
import requests
from .settings import SENSORS_AFRICA_API, SENSORS_AFRICA_AUTH_TOKEN, OWNER_ID
...
def get_sensors_africa_locations():
    response = requests.get(f"{SENSORS_AFRICA_API}/v2/locations/", headers={"Authorization": f"Token {SENSORS_AFRICA_API_KEY}"})
    if response.ok:
        """
